...
inline matrix2D::matrix2D(char flag)
{
	if (flag == 'I')
	{
		mxOrigin[0][0] = 1;
...
//Copy Constructor
inline matrix2D::matrix2D(const matrix2D &other)
{
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 3; ++j)
		{
...
//Get
inline float matrix2D::getmxComp(int x, int y)
{
	if (x > 2 || y > 2)
	{
...
//Set
inline void matrix2D::setmxComp(int x, int y, float value)
{
	mxOrigin[x][y] = value;
}

//Transformation of 2D vector
inline vector2D matrix2D::vectorToMatrix(vector2D other)
{
	vector2D vecMx(0, 0);
...
inline matrix2D matrix2D::transpose(matrix2D &other)
{
	return matrix2D(other.mxOrigin[0][0], other.mxOrigin[1][0], other.mxOrigin[0][1], other.mxOrigin[1][1]);
}

//Rotation matrix
inline matrix2D matrix2D::mxRot(float angle)
{
...
//Operator *
inline vector2D matrix2D::operator*(const vector2D &other)
{
	vector2D ockelbo;
...
//Operator *
inline matrix2D matrix2D::operator*(matrix2D other)
{
...
//Translation Matrix
inline matrix2D matrix2D::translationMatrix(vector2D pos)
{
...
//Set 3D
inline void matrix2D::setPosition(vector2D pos)
{
	mxOrigin[2][0] = pos.vecGet(0);
	mxOrigin[2][1] = pos.vecGet(1);
}

inline vector2D matrix2D::getPosition()
{
